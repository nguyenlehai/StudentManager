...
namespace exam.Controllers { [Route("api/learning-capacities")]
	public class LearningCapacitiesController: Controller {
		...
		public async Task < IActionResult > GetList() {
			...
			if (nations == null || !nations.Any()) return NotFound(new {
				message = "Không có học lực nào"
			});
			return Ok(new {
		...
		public async Task < IActionResult > GetItem(int id) {
			...
			if (nation == null) return NotFound(new {
				...
				message = "Không tìm thấy học lực"
			});
			return Ok(new {
				status = ResultStatus.STATUS_OK,
				data = nation
		...
		public async Task < IActionResult > CreateItem([FromBody] LearningCapacity fromBody) {
			if (String.IsNullOrEmpty(fromBody.Name)) return BadRequest(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Tên học lực không được để trống"
			});
			if (fromBody.MinPoint ==
		default(float))
		...
		public async Task < IActionResult > PutItem(int id, [FromBody] LearningCapacity fromBody) {
			...
			return Ok(new {
				status = ResultStatus.STATUS_OK,
				message = "Sửa thông tin học lực thành công",
				data = exist
			});
		}
