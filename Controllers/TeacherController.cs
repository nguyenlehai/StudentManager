...
namespace StudentManager.Controllers {
	...
	public class TeacherController: Controller {
		...
		public async Task < IActionResult > List() {
			...
			if (list == null || !list.Any()) return NotFound(new {
		...
		public async Task < IActionResult > Create([FromBody] Teacher teacher) {
			if (String.IsNullOrEmpty(teacher.Name)) return BadRequest(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Tên giáo viên không được để trống"
			});
			if (String.IsNullOrEmpty(teacher.Phone)) return BadRequest(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Số điện thoại giáo viên không được để trống"
			});
			if (String.IsNullOrEmpty(teacher.Address)) return BadRequest(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
			...
			if (tmp2 != null) {
				return Ok(new {
					...
					message = "Số điện thoại đã có người đăng kí"
				});
			}
		...
		[Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard")][HttpPut]
		...
		public async Task < IActionResult > Delete(int id) {
			...
			if (teacher == null) return NotFound(new {
				status = ResultStatus.STATUS_NOT_FOUND,
				message = "Không tìm thấy giáo viên"
			});
