...
namespace exam.Controllers {
	...
	public class AuthController: Controller {
		...
		[Authorize(Roles = "Admin")][HttpGet][Route("roles")]
		...
		public async Task < IActionResult > SignUp([FromBody] UserSignUp newU) {
			if (String.IsNullOrEmpty(newU.Name)) return Ok(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Thiếu họ tên người dùng"
			});
			if (String.IsNullOrEmpty(newU.Username) || String.IsNullOrEmpty(newU.Password)) return Ok(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Thiếu tên đăng nhập hoặc mật khẩu"
			});
			if (String.IsNullOrEmpty(newU.Password) || String.IsNullOrEmpty(newU.Password)) return Ok(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Mật khẩu không được để trống"
			});
			if (newU.Password.Length < 6) return Ok(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Mật khẩu không được ngắn hơn 6 kí tự"
			});
			...
			if (newU.Role.Equals(RoleCode.ROLE_ADMIN)) {
				return Ok(new {
					status = ResultStatus.STATUS_FOBIDDEN,
					message = "Không thể tạo tài khoản mang quyền Admin"
				});
			}
		...
		} [Authorize(Roles = "Admin")][HttpPost("assign")]
		public async Task < IActionResult > AssignRole([FromBody] List < PostAssignRole > fromBody) {
			if (fromBody == null || !fromBody.Any()) return BadRequest(new {
		...
		private string GenToken(User u) {
			...
			claims: claims, expires: DateTime.Now.AddDays(1), signingCredentials: new SigningCredentials(key, SecurityAlgorithms.HmacSha256));
		...
		public async Task < ActionResult > UserInfo() {
			...
			if (u == null) return BadRequest(new {
				message = "Không tìm thấy người dùng"
			});
			return Ok(new {
				status = ResultStatus.STATUS_OK,
