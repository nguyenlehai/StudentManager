...
namespace StudentManager.Controllers { [Route("api/point")]
	public class PointController: Controller {
		...
		public async Task < int > XepLoaiHocLucMonHocAsync(float[] arrayDiemTBTungMon, float tongDiem) {
			...
			for (int i = 0; i < arrayDiemTBTungMon.Length - 1; i++) {
				if (arrayDiemTBTungMon[i] < diemTBMonNhoNhat) diemTBMonNhoNhat = arrayDiemTBTungMon[i];
			}
			...
			if (xepLoai == -1) xepLoai = maHocLuc[count - 1];
			return xepLoai;
		}
		...
		#region Danh sách loạiđiểm(Kiểm tra miệng, 15p, 1 tiết, học kì...)[Authorize(Roles = "SchoolBoard")][HttpGet("point-types")]
		public async Task < ActionResult > GetPointTypes() {
			List < PointType > pointtypes = await pointTypeRepository.GetAll();
			if (pointtypes == null || pointtypes.Count() == 0) return Ok(new {
				status = ResultStatus.STATUS_NOT_FOUND,
				message = "Không có loại điểm nào"
			});
			else return Ok(new {
		...
		public async Task CreateOrUpdate([FromBody] PostUpdatePoint givenPoint) {
			foreach(ListPoint ls in givenPoint.ListPoints) {
				if (ls.Id ==
			default(int)) {#region Tạo mớiđiểm
			...
			await semesterResultBySubjectRepository.Save(this, givenPoint.StudentId, givenPoint.ClassId, givenPoint.SubjectId, givenPoint.SemesterId, givenPoint.SchoolYearId);

			await yearResultBySubjectRepository.Save(this, givenPoint.StudentId, givenPoint.ClassId, givenPoint.SubjectId, givenPoint.SchoolYearId);

			await semesterResultRepository.Save(this, conductRepository, givenPoint.StudentId, givenPoint.ClassId, givenPoint.SemesterId, givenPoint.SchoolYearId);

			await yearResultRepository.Save(this, givenPoint.StudentId, givenPoint.ClassId, givenPoint.SchoolYearId);

		} [HttpPost("update")]
		public async Task < ActionResult > CreateOrUpdate([FromBody] List < PostUpdatePoint > points) {
			if (PointController.maxPoint ==
		default(float)) {
				Rule rule = await ruleRepository.GetDefaultRule();
				maxPoint = rule.MaxPoint ==
			default(float) ? 0 : rule.MaxPoint;
			}
		...
		public async Task < ActionResult > ShowStudentPoint([FromBody] PostUpdatePoint given) //Không dùng PointNumber
		{
			...
			return NotFound(new {
				message = "Học sinh chưa có điểm"
			});
			List < dynamic > ls = new List < dynamic > ();
			foreach(var v in list) {
				ls.Add(new {
					name = v.PointType.Name,
					point = v.PointNumber
				});
			}
		...
		public async Task < float > DiemTrungBinhKiemTra(int studentId, int maMonHoc, int semesterId, int schoolYearId, int classId) {
			...
			if (list != null && list.Any()) {
				...
				if (tongHeSo > 0) return tongDiem / tongHeSo;
				else return 0;
			}
			return 0;
		}
