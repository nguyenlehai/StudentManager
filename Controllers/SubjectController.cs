...
namespace StudentManager.Controllers { [Route("api/subject")]
	public class SubjectController: Controller {
		...
		public async Task < IActionResult > GetList() {
			...
			if (subjects == null || !subjects.Any()) return NotFound(new {
		...
		public async Task < IActionResult > GetItem(int id) {
			...
			if (subject == null) return NotFound(new {
		...
		public async Task < IActionResult > Create([FromBody] Subject subject) {
			if (String.IsNullOrEmpty(subject.Name)) return BadRequest(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Tên môn học không được để trống"
			});
			if (subject.LessionSize ==
		default(int))
