...
namespace StudentManager.Controllers { [Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard,Admin")][Route("api/rule")]
	public class RuleController: Controller {
		...
		public async Task < ActionResult > GetRule() {
			...
			if (rule == null) return NotFound(new {
		...
		default(int)) {
				...
				if (existRule == null) {
					...
					rule.Id = 1;
					if (rule.MaxAge !=
				default(int)) existRule.MaxAge = rule.MaxAge;
					if (rule.MinAge !=
				default(int)) existRule.MinAge = rule.MinAge;
					if (rule.MaxSize !=
				default(int)) existRule.MaxSize = rule.MaxSize;
					if (rule.MinSize !=
				default(int)) existRule.MinSize = rule.MinSize;
					if (rule.MaxPoint !=
				default(int)) existRule.MaxPoint = rule.MaxPoint;
					if (rule.SchoolName !=
				default(string)) existRule.SchoolName = rule.SchoolName;
					if (rule.SchoolAddress !=
				default(string)) existRule.SchoolAddress = rule.SchoolAddress;
