...
namespace exam.Controllers { [Route("api/students")]
	public class StudentController: Controller {
		...
		[Authorize(Roles = "SchoolStaff,SchoolBoard")][HttpGet]
		public async Task < IActionResult > PostAsync() {
			...
			return Ok(response);
		} [Authorize(Roles = "SchoolStaff,SchoolBoard,Teacher")][HttpGet("find")][Microsoft.AspNetCore.Authorization.Authorize]
		public async Task < ActionResult > FindStudentByName(string name) {
			...
			if (students == null || students.Count() == 0) return Ok(new {
		...
		[HttpGet("class/{classId}")][Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard,SchoolStaff")]
		...
		[Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard,SchoolStaff,Teacher")][HttpGet("{id}")]
		...
		[HttpPost][Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolStaff,SchoolBoard")]
		public async Task < IActionResult > Create([FromBody] Student u) {
			if (String.IsNullOrEmpty(u.FullName)) return BadRequest(new {
		...
		[Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard,SchoolStaff")][HttpPut("{id}")]
		public async Task < IActionResult > Edit(int id, [FromBody] Student student) {
			...
			if (String.IsNullOrEmpty(student.FullName)) return Ok(new {
				status = ResultStatus.STATUS_INVALID_INPUT,
				message = "Tên học sinh không được để trống"
			});
			...
			return Ok(new {
				status = ResultStatus.STATUS_OK,
				message = "Sửa thông tin học sinh thành công",
				data = student
			});
		}

		[HttpDelete("{id}")][Microsoft.AspNetCore.Authorization.Authorize(Roles = "SchoolBoard,SchoolStaff")]
